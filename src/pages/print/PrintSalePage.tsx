...
import { v1SalesRetrieveOptions, v1CompanyRetrieveOptions } from '../../client/@tanstack/react-query.gen';
...
export default function PrintSalePage() {
  ...
  useEffect(() => {
    if (sale && company && !isLoading && !hasPrinted.current) {
  ...
  if (error || !sale) {
    return <div className="p-8 text-center text-red-500">Error al cargar los datos de la venta.</div>;
  }

  const documentTitle = type === 'nota_entrega' ? 'NOTA DE ENTREGA' : 'FACTURA';
      ...
      <div className="max-w-4xl mx-auto bg-white print:p-0 print:shadow-none shadow-md rounded-lg p-8 sm:p-12">
        ...
        <div className="flex justify-between items-start border-b-2 border-gray-200 pb-6 mb-6">
          ...
          <div className="text-right">
            ...
            <p className="text-lg font-bold text-gray-600 mt-1">Nº {sale.seq_number}</p>
            <p className="text-sm text-gray-500 mt-2">
              Fecha: {format(new Date(sale.created_at), "dd/MM/yyyy")}
            </p>
            <p className="text-sm text-gray-500">
              Hora: {format(new Date(sale.created_at), "HH:mm")}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-8 mb-8">
          <div>
            <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Cobrar a:</h3>
            <p className="font-bold text-gray-900">{sale.customer_name || 'Consumidor Final'}</p>
            <p className="text-sm text-gray-600">ID / RIF: {sale.customer || 'Público General'}</p>
          </div>
          <div className="text-right">
            <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Estado de Pago:</h3>
            <p className="font-bold text-gray-900">
              {sale.payment_status === 'PAID' ? 'PAGADO' : sale.payment_status === 'PARTIALLY_PAID' ? 'PAGO PARCIAL' : 'PENDIENTE'}
            </p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {sale.sale_details.map((detail, index) => {
                ...
                const isLast = index === sale.sale_details.length - 1;
                return (
        ...
        <div className="flex justify-end">
          <div className="w-64">
            <div className="flex justify-between py-2 border-b border-gray-200">
              ...
              <span className="text-sm font-bold text-gray-900">${parseFloat(sale.total_amount_usd || '0').toFixed(2)}</span>
            </div>
            {/* If we had taxes calculated globally, we would put them here. */}
            <div className="flex justify-between py-3 border-b-2 border-gray-800">
              ...
              <span className="text-base font-black text-gray-900">${parseFloat(sale.total_amount_usd || '0').toFixed(2)}</span>
            </div>
            
            <div className="flex justify-between py-2 text-sm">
              <span className="text-gray-600">Total Pagado:</span>
              <span className="font-medium text-emerald-600">${parseFloat(sale.total_paid || '0').toFixed(2)}</span>
            </div>
            <div className="flex justify-between py-2 text-sm">
              <span className="text-gray-600">Saldo Pendiente:</span>
              <span className="font-medium text-red-600">${(parseFloat(sale.total_amount_usd || '0') - parseFloat(sale.total_paid || '0')).toFixed(2)}</span>
            </div>
          </div>
        </div>
